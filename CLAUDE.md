...
- **Dave ↔ Steve**: **no channel exists today.** Neither program has ever talked to the other. If a future feature needs Dave to push something into Steve, the established pattern to replicate is a new `tcpServer.TCPServer` inside Steve (Steve currently only runs a `tcpClient`) plus a matching `tcpClient.TCPClient` in Dave, mirroring the Hal/Kilroy client pattern above (see `prompt_history/` for a concrete investigation of this — the "draw FOV boundary in Steve" feature request).
